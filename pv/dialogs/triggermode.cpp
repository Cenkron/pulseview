...
TriggerMode::TriggerMode(MainWindow *parent) :
	QDialog(parent),
	parent_(parent),
	layout_(this),
	form_(this),
...
// sync with parent
	...
	chkbox_repetitive_->setChecked(parent_->get_capture_mode() == pv::Repetitive);
	rearm_delay_->setValue(parent_->get_repetitive_rearm_time());
}

void TriggerMode::chkbox_toggled(bool checked)
{
	parent_->set_capture_mode(checked ? pv::Repetitive : pv::Single);
}

void TriggerMode::rearm_time_changed(int value)
{
	parent_->set_repetitive_rearm_time(value);
}
